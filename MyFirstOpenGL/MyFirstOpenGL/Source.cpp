...
GLuint LoadGeometryShader(const std::string& filePath) {
	...
	// Crear un geometry shader
	GLuint geometryShader = glCreateShader(GL_GEOMETRY_SHADER);

	//Usamos la funcion creada para leer el geometry shader y almacenarlo 
	std::string sShaderCode = Load_File(filePath);
	const char* cShaderSource = sShaderCode.c_str();

	//Vinculamos el geometry shader con su código fuente
	glShaderSource(geometryShader, 1, &cShaderSource, nullptr);

	// Compilar el geometry shader
	...
	//Si la compilacion ha sido exitosa devolvemos el geometry shader
	if (success) {
